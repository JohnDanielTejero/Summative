...
public class QuestionAndAnswer extends StudentProfile{

    //Instantiation
    public static int questioncounter = 1; /*Dictate what item of the 3 arraylist appears first*/
    private static int maximumquestion;

    public static void promptQuestion(ArrayList<String> qna, ArrayList<String> choices, ArrayList<String> answers) {
        ...
        while (questioncounter <= maximumquestion){
            ...
            if (answer.length == 1){ /*If answers is one only this gets executed*/
                ...
                if ((listofans[getAns[0]].equals(answer[0]) || listofans[getAns[0]].equals(answer[1])) &&   /* If first answer is either of the 2 correct answer*/
                    ...
                    questioncounter++;
                    setMistakes(getMistakes()+ 1) /*Mistakes get incremented*/;
                }
        ...
        setFinalPoints(getMaxQuestion() - getMistakes()); /*Computes the final score before setting it as the final points*/
        setPercentScore(Math.round((Double.valueOf(getFinalPoints())/getMaxQuestion()*100)*100)/100); /*takes 2 decimal places and transforms it to percent*/
